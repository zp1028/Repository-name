...
def safe_json_get(url: str, timeout: int = 15, retries: int = 2) -> Optional[dict]:
    """带重试的JSON请求"""
    for attempt in range(retries + 1):
        try:
            r = requests.get(url, timeout=timeout, headers={"User-Agent": "Mozilla/5.0"})
...
def parse_pattern(text: str, allowed_chars: str) -> Optional[List[str]]:
    """通用形态解析：只允许 allowed_chars 中的字符，返回列表"""
...
def count_combo_hits_ssq(df: pd.DataFrame, nums: List[int], n: Optional[int] = None) -> Tuple[int, int, List[str]]:
    if n:
        df = df.tail(n)
    red_cols = [f"红球{i}" for i in range(1, 7)]
    target = set(nums)
...
def count_combo_hits_kl8(df: pd.DataFrame, nums: List[int], n: Optional[int] = None) -> Tuple[int, int, List[str]]:
    if n:
        df = df.tail(n)
    cols = [c for c in df.columns if c.startswith("号")]
    target = set(nums)
...
# ==================== 极速飞艇模块 ====================
def fetch_feiting_latest() -> Optional[Dict]:
...
def fetch_kl8_speed_latest() -> Optional[Dict]:
...
# ==================== UI辅助函数（复用） ====================
def show_combo_analysis(df: pd.DataFrame, count_func, max_num: int, label: str):
    """五码组合分析通用UI"""
...
if lottery == "双色球":
    ...
    with tab4:
        show_combo_analysis(df, count_combo_hits_ssq, 33, "ssq")
    ...
    with tab4:
        show_combo_analysis(df, count_combo_hits_kl8, 80, "kl8")
    ...
    # 自动刷新（使用 st.fragment 实现轻量轮询，仅更新指定区域）
    if auto_refresh:
        # 用 st.fragment 包装需要刷新的内容，设置 run_every
        @st.fragment(run_every=45)
        def auto_refresh_fragment():
            # 重新加载数据（缓存可能过期）
            df_new = load_feiting(ft_days)
            if not df_new.empty:
                st.session_state["feiting_df"] = df_new
            # 显示最新数据块（略，但无法通过 fragment 覆盖外部变量，所以我们将整个飞艇主逻辑放入 fragment）
            # 由于重构复杂，此方案暂简化为 meta 刷新提示
            st.info("⏱ 自动刷新已启用（约每45秒），页面将整体刷新。")
        # 为简化，我们保留 meta 刷新，但提示用户
